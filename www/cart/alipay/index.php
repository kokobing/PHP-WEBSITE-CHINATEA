<?php
/* *
 * 功能：支付宝XXXXXX接口调试入口页面
 * 版本：3.2
 * 日期：2011-03-17
 * 说明：
 * 以下代码只是为了方便商户测试而提供的样例代码，商户可以根据自己网站的需要，按照技术文档编写,并非一定要使用该代码。
 */
$mainname="WWW.一品茶.COM";    //网站名称
?>
...
<body text=#000000 bgColor=#ffffff leftMargin=0 topMargin=4>
	<div id="main">
        ...
        <form name=alipayment onSubmit="return CheckForm();" action=alipayto.php method=post target="_blank">
              <input type="hidden" name="payname"  id="payname" value="<?=$_POST[payname];?>">
              <input type="hidden" name="pdtname"  id="pdtname" value="<?=$_POST[pdtname];?>">
              <input type="hidden" name="payaddr"  id="payaddr" value="<?=$_POST[payaddr];?>">
              <input type="hidden" name="paymobi"  id="paymobi" value="<?=$_POST[paymobi];?>">
              <input type="hidden" name="paytel"   id="paytel"  value="<?=$_POST[paytel];?>">
            <div id="body" style="clear:left">
                <dl class="content">
                    <dt>订单号：</dt>
                    <dd>
                        <span class="red-star">*</span>
                        <input size=30 name=subject value="<?=$_POST[ordercode]?>" readonly="readonly"/>
                        <span>付款到：<?php echo $mainname;?>&nbsp;</span>
                    </dd>
                    <dt>付款金额：</dt>
                    <dd>
                        <span class="red-star">*</span>
                        <input maxLength=10 size=30 name=total_fee onfocus="if(Number(this.value)==0){this.value='';}" value="<?=$_POST[totalpaymoney]?>"  readonly="readonly"/>
                        <span>如：112.21</span>
                    </dd>
